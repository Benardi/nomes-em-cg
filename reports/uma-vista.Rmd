...
```{r}
vias %>%
    filter(arvores_100m_mean < 5) %>%
    ggplot(aes(pontomotot, y=..count.. / sum(..count..))) +
    geom_bar()
```

```{r}
vias %>%
    ggplot(aes(arvores_100m_mean,..density..)) +
    geom_histogram(binwidth = 2.5)
```

```{r}
vias %>%
    ggplot(aes(comprimento,..density..)) +
